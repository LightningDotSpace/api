...
export class AsyncCache<T> {
  ...
  async get(
    ...
    if (!cachedEntry) throw new Error(`Error in AsyncCache: cached entry for id ${id} is null`);
  ...
  private async updateInternal(id: string, update: () => Promise<T>, fallbackToCache: boolean) {
    try {
      ...
      const updateCall = update()
        ...
        .finally(() => {
          const cachedEntry = this.cache.get(id);
          if (cachedEntry) this.cache.set(id, { ...cachedEntry, update: undefined });
        });

      if (entry) this.cache.set(id, { ...entry, update: updateCall });
